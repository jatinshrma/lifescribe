...
export default function CustomAutocomplete() {
	const [selectedFilms, setSelectedFilms] = useState<FilmOptionType[]>([])
	const [query, setQuery] = useState("")

	const filteredFilms =
		!query || query?.length < 2
			? []
			: top100Films.filter(film => film.title.toLowerCase().includes(query.toLowerCase()))
	...
	return (
		<div className="mt-3 relative w-full space-y-2">
			<Combobox
				onChange={value => {
					setQuery("")
					if (value && selectedFilms?.length <= 10) setSelectedFilms(prev => prev.concat([value]))
				}}
			>
				...
				>
					{query?.length > 2 &&
						filteredFilms?.map(film => (
							...
							>
								<div className="text-sm text-white">{film?.title}</div>
							</ComboboxOption>
						))}
				</ComboboxOptions>
			</Combobox>
			{selectedFilms?.length > 0 && (
				<div className="flex gap-2 flex-wrap">
					{selectedFilms.map(film => (
						<div key={film?.title} className="theme-button static small rounded-md bg-darkSecondary w-fit">
							{film?.title}
							<BiX
								className="ml-1 h-5 w-5 cursor-pointer"
								onClick={() =>
									setSelectedFilms(prev => prev.filter(selectedFilm => selectedFilm.title !== film?.title))
								}
							/>
						</div>
					))}
